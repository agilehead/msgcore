/**
 * MsgCore Test Database
 *
 * Manages the msgcore.db for integration testing.
...
import { createLogger, type Logger } from "@agilehead/msgcore-logger";

const __filename = fileURLToPath(import.meta.url);
const __dirname = dirname(__filename);

// Get project root (4 levels up from this file: src -> msgcore-test-utils -> packages -> node -> msgcore)
...
export function createTestDatabase(
  ...
  const log = logger ?? createLogger("msgcore-test-db");
  ...
  if (externalDbPath !== undefined) {
    ...
    dbPath = join(testDir, "db", "msgcore.db");
  }
...
export async function setupTestDatabase(testDb: TestDatabase): Promise<void> {
  ...
  if (!testDb.isExternal) {
    ...
    testDb.knex = Knex({
      ...
      migrations: {
        directory: join(PROJECT_ROOT, "database/msgcore/migrations"),
      },
    });
